...
const TONE: Record<FeedbackMessage['kind'], string> = {
  // 浅色模式用深色文字（浅淡背景上清晰），深色模式保持浅色文字（dark: 由 html.dark 驱动）。
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-[52px] z-[100] flex justify-center px-4">
      <div
        ...
        className={cn(
          'global-feedback-slot pointer-events-auto relative flex w-fit max-w-[min(560px,calc(100vw-32px))] items-start gap-2.5 rounded-md border py-2 pr-9 pl-3 text-xs shadow-2xl backdrop-blur',
          'transition-all duration-200 ease-out',
          leaving
            ? '-translate-y-2 scale-[0.98] opacity-0'
            : 'translate-y-0 scale-100 animate-[global-feedback-in_220ms_ease-out] opacity-100',
          TONE[feedback.kind]
        )}
      >
        <Icon size={15} className="mt-0.5 shrink-0" />
        <span className="min-w-0 flex-1 leading-5 break-words whitespace-pre-wrap">{feedback.message}</span>
        <button
          type="button"
          aria-label="关闭提示"
          onClick={close}
          className="absolute top-2 right-2 rounded p-1 text-current/70 transition-colors hover:bg-black/10 hover:text-current dark:hover:bg-white/10"
        >
          <XIcon size={13} />
        </button>
      </div>
