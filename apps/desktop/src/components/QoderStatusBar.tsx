...
export function QoderStatusBar({
  status,
  refreshing,
  onRefresh
}: {
      ...
      <div className="flex min-w-0 flex-1 items-center gap-4 overflow-hidden">
        {hasError ? (
          ...
          >
            ...
            {status.connected ? (status.running ? 'Qoder 执行中' : 'Qoder 已连接') : 'Qoder 未连接'}
          </span>
        )}
        <span className="shrink-0">
          档位 <b className="text-foreground">{tier}</b>
        </span>
        {defaultModel && (
          <span className="shrink-0">
            默认模型 <b className="text-foreground">{defaultModel}</b>
          </span>
        )}
        {quotaUsed !== undefined && (
          <span className="shrink-0">
            ...
            <b className="text-foreground">
              {formatTokens(quotaUsed)}
              {quotaTotal !== undefined ? ` / ${formatTokens(quotaTotal)}` : ''}
              {quotaUnit ? ` ${quotaUnit}` : ''}
            </b>
          </span>
        )}
        {percentage !== undefined && (
          <>
            ...
            <span>
              <b className="text-foreground">{percentage.toFixed(1)}%</b>
            </span>
          </>
        )}
        {status.usage?.addOnQuota?.remaining !== undefined && (
          <span>
            加量包剩余 <b>{formatTokens(status.usage.addOnQuota.remaining)}</b>
          </span>
        )}
        {status.usage?.isQuotaExceeded && <span className="text-red-300">配额已用尽，仅 lite 免费模型可用</span>}
      </div>
      <Button
        variant="ghost"
        size="icon-sm"
        title={refreshing ? '正在刷新 Qoder 状态' : '刷新 Qoder 状态'}
        aria-label="刷新 Qoder 状态"
        disabled={refreshing}
        onClick={onRefresh}
      >
        <RefreshCwIcon className={refreshing ? 'animate-spin-slow' : ''} size={12} />
      </Button>
    </footer>
  )
}
