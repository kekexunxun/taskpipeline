...
describe('OpenAIMessageView', () => {
  it('renders text parts through the shared TextPart renderer', () => {
    render(
      <OpenAIMessageView message={openaiMessage([{ driverId: 'openai', type: 'text', text: 'OpenAI 的纯文本回复' }])} />
    )
  ...
  it('renders markdown in text parts via the same renderer as Qoder', () => {
    render(
      <OpenAIMessageView
        message={openaiMessage([{ driverId: 'openai', type: 'text', text: '# 标题\n\n- 列表项 1\n- 列表项 2' }])}
      />
    )
  ...
  it('renders an openai message alongside an openai text part (clean basic case)', () => {
    ...
    render(
      <OpenAIMessageView message={openaiMessage([{ driverId: 'openai', type: 'text', text: 'OpenAI 的简洁回复' }])} />
    )
