...
vi.mock('./EditorLauncher', () => ({
  EditorLauncher: () => (
    <button type="button" aria-label="打开文件夹">
      📁
    </button>
  )
}))
...
const renderHeader = (task: Task, repositories: TaskRepository[] = []) =>
  render(
...
describe('DetailHeader', () => {
  it('shows the full task description inside a clamped container with a section header when not expanded', () => {
    ...
    expect(block?.className).toContain('max-h-22')
  ...
  it('collapses acceptance criteria when the list exceeds the threshold with a text toggle', async () => {
    const user = userEvent.setup()
    const { container } = renderHeader({
      ...baseTask,
