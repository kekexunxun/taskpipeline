import type { SessionUsage, Task, TaskCard } from '@task-pipeline/core'
...
export function UsageSection({
  ...
  const stats = usage
    ? [
        ['总 Token', formatTokens(usage.totalTokens)],
        ['输入', formatTokens(usage.inputTokens)],
        ['输出', formatTokens(usage.outputTokens)],
        ...(usage.costUsd !== undefined ? [['费用', `$${usage.costUsd.toFixed(usage.costUsd < 0.01 ? 4 : 2)}`]] : []),
        ...(usage.durationMs !== undefined ? [['耗时', formatDuration(usage.durationMs)]] : []),
    ...
    <section className="border-b px-5 py-3">
      <div className="mb-2 flex items-center justify-between text-xs font-semibold text-muted-foreground">
        ...
        <Badge variant="outline">
          {usage?.provider === 'qoder' ? 'Qoder' : usage ? 'OpenAI-Compatible' : '等待数据'}
        </Badge>
      </div>
      {hasModels ? (
        ...
        <div className="grid grid-cols-4 gap-2">
          {stats.map(([label, value]) => (
            <span className="min-w-0" key={label}>
              ...
              <b className="block truncate font-mono text-xs text-foreground">{value}</b>
            </span>
          ))}
        </div>
      ) : (
