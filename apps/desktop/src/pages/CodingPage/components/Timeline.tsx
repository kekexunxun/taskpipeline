...
export function Timeline({ items }: { items: TimelineItem[] }) {
  ...
  return <div className="px-5 py-4 pb-16">
    ...
    {visibleItems.map((item) => { const Icon = icons[item.kind]; const payload = item.kind === "review" ? (item as AgentEvent).payload as { comments?: Array<{ severity?: string; path?: string; line?: number; message?: string }> } | undefined : undefined; const reviewComments = payload?.comments?.filter((comment) => comment.message || comment.path) ?? []; const severityBadge = (severity?: string) => { const level = String(severity ?? "").toLowerCase(); return <Badge variant={level === "critical" || level === "high" || level === "error" ? "destructive" : level === "medium" ? "warning" : level === "low" ? "secondary" : "outline"}>{level || "info"}</Badge>; }; return <article className="mb-4 grid grid-cols-[26px_minmax(0,1fr)] gap-2" key={item.id}><div className={cn("grid size-6 place-items-center rounded-full border bg-muted text-muted-foreground", item.kind === "error" && "border-red-500/30 text-red-300")}><Icon size={12} /></div><div className="min-w-0 pt-0.5"><div className="flex items-center justify-between gap-3"><strong className="text-xs font-medium">{localizedEventTitle(item.title)}</strong><time className="shrink-0 text-xs text-muted-foreground">{formatTime(item.createdAt)}</time></div>{item.detail && <pre className="thin-scrollbar mt-1.5 overflow-x-auto whitespace-pre-wrap rounded-md border bg-background p-2 font-mono text-xs leading-4 text-muted-foreground">{item.detail}</pre>}{reviewComments.length > 0 && <div className="mt-1.5 space-y-1">{reviewComments.map((comment, index) => <div className="rounded-md border bg-background px-2 py-1.5 text-xs" key={index}><div className="flex items-center gap-1.5">{severityBadge(comment.severity)}{comment.path && <span className="font-mono text-[11px] text-muted-foreground">{comment.path}{typeof comment.line === "number" ? `:${comment.line}` : ""}</span>}</div>{comment.message && <p className="mt-1 whitespace-pre-wrap break-words leading-4 text-muted-foreground">{comment.message}</p>}</div>)}</div>}</div></article>; })}
    <div ref={endRef} />
  </div>;
}
