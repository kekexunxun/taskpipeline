    ...
    <div className="mx-auto w-full max-w-4xl">
      <div className={cn("mb-3 flex items-center justify-between font-semibold", compact ? "text-xs" : "text-sm")}>
        <span
          ...
          className={cn(
            "inline-flex items-center gap-1.5 text-xs text-muted-foreground animate-plan-fade-up",
            isRevising && "text-foreground/80"
          )}
        >
          {isRevising && <span className="relative inline-flex h-1.5 w-1.5">
        ...
        <div className={cn("mb-4 border-l-2 border-border", compact ? "pl-3" : "pl-4")}>
          ...
          <div className="space-y-2" data-plan-feedback>
            {planFeedback.map((item, index) => (
              <div
                className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 text-xs animate-plan-fade-up"
                key={item.id}
      ...
      {isRevising && (
        <div
          ...
          className="mb-3 flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-amber-200 animate-plan-fade-up"
        >
      ...
      {planContent ? (
        <div
          ...
          className={cn("rounded-md border bg-background text-xs leading-5 animate-plan-fade-up", compact ? "p-3" : "p-5")}
        >
        ...
        <div
          ...
          aria-live="polite"
        >
          <div className="pointer-events-none absolute inset-0 plan-shimmer-overlay animate-plan-shimmer" aria-hidden />
        ...
        <div className="rounded-md border border-dashed p-8 text-center text-xs text-muted-foreground">等待生成计划</div>
      )}
    </div>
  </section>;
}
