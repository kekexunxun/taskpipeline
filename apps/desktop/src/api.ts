...
export type DriverPart =
  ...
  | {
      driverId: ChatDriverId
      /** Chat 每轮 CodeReview 结论卡（主进程评审编排产出）。 */
 ...
 * 存储于 dataDir/plans/{chatId}/{planId}.md，执行时告知 LLM 文件路径避免上下文爆炸。
 */
...
/** 对话模式：normal=常规对话, plan=只读计划模式。后续可扩展更多模式。 */
...
export type ChatMessageMetadata = {
  ...
  errorMessage?: string
  /** 计划模式标记：为 true 时 PartRenderer 将文本内容渲染为 PlanCard（显示“计划生成中...”）。 */
  isPlanMode?: boolean
  /**
   * 计划等待标记（仅内存、不持久化）：plan-part 到达时置 true，表示对话正处于
   * “等待用户处理计划”的存活态；会话重载/应用重启后标记丢失 = 对话已结束。
   */
  planWaiting?: boolean
}
