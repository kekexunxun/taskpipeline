...
async function runQoder(taskId: string, extraPrompt?: string, signal?: AbortSignal): Promise<void> {
...
async function advanceAfterValidation(taskId: string, state: TaskState, signal?: AbortSignal): Promise<void> {
  ...
  if (updated?.state === "awaiting_commit" && taskWorkflow.shouldAutoCreateMergeRequests()) await deliveryService.submitMergeRequests(taskId, signal);
}
...
async function stopTaskOperations(taskId: string, markFailed: boolean): Promise<void> {
  ...
  const task = store.getTask(taskId);
  if (markFailed && task && ["planning", "implementing", "validating"].includes(task.state)) updateState(task, "failed");
  ...
  if (activeTaskId === taskId) {
    ...
    try { await qoderQuery?.close(); } catch { /* The query may already be closed. */ }
    await stopPi();
  }
