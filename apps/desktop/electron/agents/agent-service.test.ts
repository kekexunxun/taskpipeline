...
function makeService(initial: Array<Record<string, unknown>> = [], listWikiDocs?: (repositoryId: string) => RepoWikiDoc[]) {
...
describe("AgentService CRUD", () => {
  it("lists 3 role agents by default and persists via save", () => {
    ...
    expect(list.map((a) => a.id).sort()).toEqual(["builtin-reviewer", "builtin-test-writer", "builtin-mr-writer"].sort());
  ...
  it("delete removes the agent without affecting others and tolerates missing ids", () => {
    ...
    service.save(b);
    service.delete(a.id);
    expect(service.list().filter((item) => !item.builtin).map((item) => item.id)).toEqual([b.id]);
...
describe("AgentService resolveAgentFor", () => {
  it("matches enabled agents by repository whitelist, newest update wins", () => {
    ...
    service.save({ ...createAgentDraft("禁用"), repositoryIds: ["r1"], enabled: false, updatedAt: "2026-03-01T00:00:00.000Z" });
...
describe("AgentService resolveRuntime", () => {
  it("prefers task.qoderModel over agent preference", () => {
    const service = makeService();
    service.save({ ...createAgentDraft("A"), repositoryIds: ["r1"], preferredProvider: "openai", preferredModel: "gpt-5" });
    expect(service.resolveRuntime(task("qoder:claude-sonnet-4.5"), [repo])).toEqual({
      provider: "qoder",
...
describe("AgentService wikiIncludePaths", () => {
  it("injects matched wiki docs verbatim into the agent section", async () => {
    const service = makeService([], (repositoryId) =>
      repositoryId === "r1"
        ? [{ id: "w1", repositoryId: "r1", path: "AGENTS.md", title: "Agent Instructions", content: "领域规范全文", hash: "h1", updatedAt: "" }]
        : []
    );
    service.save({ ...createAgentDraft("Java"), repositoryIds: ["r1"], systemPrompt: "约定", wikiIncludePaths: ["AGENTS.md"] });
    const { sections } = await service.resolveAgentContext(task(), [repo]);
    expect(sections).toHaveLength(1);
  ...
  it("skips wiki docs not listed in wikiIncludePaths", async () => {
    const service = makeService([], () => [
      { id: "w1", repositoryId: "r1", path: "repowiki/architecture.md", title: "架构", content: "不应出现", hash: "h1", updatedAt: "" }
    ]);
    service.save({ ...createAgentDraft("Java"), repositoryIds: ["r1"], systemPrompt: "约定", wikiIncludePaths: ["AGENTS.md"] });
    const { sections } = await service.resolveAgentContext(task(), [repo]);
    expect(sections[0]).toContain("约定");
