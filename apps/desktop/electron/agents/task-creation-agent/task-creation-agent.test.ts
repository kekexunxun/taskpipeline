...
describe('JiraTaskCreationAgent', () => {
  ...
  it('prefers a create-metadata tool exposed by Jira MCP', async () => {
    const { agent, calls } = setup({
      tools: [
        { name: 'jira_get_create_issue_metadata', inputSchema: { properties: { project_key: {}, issue_type_id: {} } } }
      ],
      result: { content: [{ type: 'text', text: JSON.stringify({ fields: { summary: { required: true } } }) }] }
    })
    await expect(agent.getCreationSchema({ projectKey: 'PAY', issueTypeId: '10001' })).resolves.toMatchObject({
      source: 'mcp:jira_get_create_issue_metadata'
    })
    expect(calls[0]).toEqual({
      name: 'jira_get_create_issue_metadata',
      args: { project_key: 'PAY', issue_type_id: '10001' }
    })
  })
  ...
  it('reports missing Jira configuration without starting MCP', async () => {
    ...
    await expect(
      agent.createJiraIssue({ projectKey: 'BSADAPT344', issueTypeName: '任务', summary: 'x' })
    ).rejects.toThrow('未配置 Jira MCP')
  })
})
    ...
    const factory = {
      ...
      create: vi.fn(() => ({
        ...
        callTool: vi.fn(async () => ({
          content: [{ type: 'text', text: JSON.stringify({ issueKey: 'BSADAPT344-7' }) }]
        })),
        close: vi.fn()
      }))
    } as unknown as AtlassianClientFactory
