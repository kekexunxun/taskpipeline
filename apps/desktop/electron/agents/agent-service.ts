...
const ROLE_AGENT_DEFAULTS: Record<
  OperationKind,
...
export class AgentService {
  constructor(
    ...
    /**
     * 系统默认模型解析器（运行时动态回填用，不落盘）。
     * Agent / 任务都未显式配置模型时，resolveRuntime 用它回填 provider + model，
     ...
     */
    private readonly isModelAvailable?: (model: string) => boolean
  ) {}
   ...
   *
   * 任一环节显式配置的模型经 isModelAvailable 校验失效时，同样回落到系统默认。
   */
  resolveRuntime(task: Task, repos: TaskRepository[]): AgentRuntime {
    const agent = this.resolveAgentFor(
      repos[0]?.repositoryId ?? '',
      task.agentProfileId,
      task.repoAgentIds?.[repos[0]?.repositoryId ?? '']
    )
    ...
    // 运行时动态回填系统默认（不写 agentProfiles，系统默认变更后自动跟随）。
    const system = this.resolveSystemModel?.()
    if (system) return { provider: system.provider, model: system.model, agent }
    return { agent }
  }

  /** 任务模型覆盖：仅当任务显式指定或 Agent 配置了成对的 provider+model 时返回；失效值回退系统默认。 */
  resolveModelForTask(task: Task, repos: TaskRepository[]): string | undefined {
    return this.resolveRuntime(task, repos).model
  }
   ...
   */
  resolveOperationAgent(
    operation: OperationKind,
    task?: Task,
    repos?: TaskRepository[]
  ): { roleAgent: AgentProfile | undefined; roleBody: string; contextBody: string } {
    ...
    if (task && repos && operation !== 'review') {
      const parts: string[] = []
      for (const repo of repos) {
        const contextAgent = this.resolveAgentFor(
          repo.repositoryId,
          task.agentProfileId,
          task.repoAgentIds?.[repo.repositoryId]
        )
