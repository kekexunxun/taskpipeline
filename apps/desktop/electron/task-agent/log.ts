...
export function recordQoderMessage(
  ...
  taskId: string,
  message: SDKMessage,
  options: { recordText: boolean; addTaskEvent: (event: { taskId: string; kind: "message" | "status" | "error"; title: string; detail?: string }) => void; emitPi: (event: { type: "qoder_event"; taskId: string; message: SDKMessage }) => void }
): void {
  ...
  else if (message.type === "system") options.addTaskEvent({ taskId, kind: "status", title: `Qoder ${message.subtype}`, detail: JSON.stringify(message).slice(0, 2000) });
