...
export function parseExtractedMemories(text: string, allowedScopes: ExtractedMemoryDraft["scope"][]): ExtractedMemoryDraft[] {
  ...
  try {
    ...
    for (const raw of parsed.memories) {
      ...
      const tags = Array.isArray(item.tags) ? item.tags.filter((tag): tag is string => typeof tag === "string").map((tag) => tag.trim()).filter(Boolean).slice(0, 8) : [];
