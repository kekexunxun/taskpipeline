...
import { JiraTaskCreationAgent, type JiraCreateInput } from '../../agents/task-creation-agent/task-creation-agent.js'
...
const createIssueShape = {
  ...
  additionalFields: z
    ...
    .optional()
    .describe('仅允许放入创建 Schema 明确返回的 customfield 字段')
} as const
    ...
    const tools: ToolDeclaration[] = [
      {
        ...
        execute: async (input) =>
          agent.getCreationSchema(input as { projectKey?: string; issueTypeId?: string; issueTypeName?: string })
      },
      {
...
export class JiraTaskCreationBackend implements TaskCreationBackend {
  ...
  get configured(): boolean {
    return this.agent.jiraConfigured
  }
