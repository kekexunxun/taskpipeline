...
function modelToolResult(value: unknown) {
...
function qoderTaskServer(backend: TaskCreationBackend, onCreated: (result: TaskCreatedResult) => void) {
  ...
  if (agent.confluenceConfigured) {
    tools.push(
      qoderTool("search_confluence", "只读搜索 Confluence，用于补充创建 Jira 所必需的内部背景。", { query: z.string(), limit: z.number().int().min(1).max(20).optional() }, async (input) => modelToolResult(await agent.searchConfluence(input)), { annotations: { readOnlyHint: true }, permissionPolicy: "always_allow" }),
      qoderTool("get_confluence_page", "只读获取 Confluence 页面正文。", { pageId: z.string() }, async (input) => modelToolResult(await agent.getConfluencePage(input)), { annotations: { readOnlyHint: true }, permissionPolicy: "always_allow" })
    );
  }
