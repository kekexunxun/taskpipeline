...
function inputProperties(tool: McpToolDefinition): Record<string, { type?: string; items?: { type?: string } }> { return tool.inputSchema?.properties ?? {}; }
...
function assignFirst(target: Record<string, unknown>, properties: Record<string, unknown>, names: string[], value: unknown): void {
...
export class JiraTaskCreationAgent {
  ...
  get jiraConfigured(): boolean { return this.factory.isConfigured("jira"); }
  get confluenceConfigured(): boolean { return this.factory.isConfigured("confluence"); }
  ...
  async createJiraIssue(input: JiraCreateInput): Promise<JiraCreatedTask> {
    ...
    if (input.taskLevelId && !this.confirmedCustomFields.has("customfield_10500")) throw new Error("任务级别字段未出现在本次创建 Schema 中");
  ...
  private getJiraClient(): McpClient { return this.jira ??= this.factory.create("jira"); }
  private getConfluenceClient(): McpClient { return this.confluence ??= this.factory.create("confluence"); }
  private async getJiraTools(): Promise<McpToolDefinition[]> { return this.jiraTools ??= await this.getJiraClient().listTools() as McpToolDefinition[]; }
  private async getConfluenceTools(): Promise<McpToolDefinition[]> { return this.confluenceTools ??= await this.getConfluenceClient().listTools() as McpToolDefinition[]; }
}
