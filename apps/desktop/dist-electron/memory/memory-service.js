...
export class MemoryService {
    ...
    upsertMemory(input) {
        if (input.id) {
            ...
            if (current)
                return this.memory.updateMemory(input.id, input);
        }
        return this.memory.createMemory({ ...input, userId: input.userId ?? this.ensureUserId() });
    }
    updateMemory(id, patch) { return this.memory.updateMemory(id, patch); }
    deleteMemory(id) { this.memory.deleteMemory(id); }
    deleteRepoMemories(repositoryId) {
        this.memory.deleteMemories({ repositoryId });
        this.memory.clearRepoWikiDocs(repositoryId);
    }
    deleteConversationMemories(conversationId) {
        this.memory.deleteMemories({ conversationId });
    }
    listRepoWikiDocs(repositoryId) { return this.memory.listRepoWikiDocs(repositoryId); }
    searchRepoWikiDocs(repositoryId, query) { return this.memory.searchRepoWikiDocs({ repositoryId, query, limit: 10 }); }
    async refreshRepoWiki(repositoryId, localPath) {
    ...
    async search(options) {
        ...
        for (const repositoryId of options.repositoryIds ?? []) {
            if (query)
                repoMemories.push(...this.memory.searchMemories({ query, scopes: ["repo"], repositoryId, limit: 3 }));
    ...
    consolidateMemories(drafts, repositoryIds, conversationId) {
        let saved = 0;
        for (const draft of drafts) {
            ...
            if (draft.scope === "repo") {
                ...
                if (this.memory.listMemories({ scope: "repo", repositoryId: primary }).some((m) => m.title.toLowerCase() === draft.title.toLowerCase()))
                    continue;
                ...
                const userId = this.ensureUserId();
                if (this.memory.listMemories({ scope: "user", userId }).some((m) => m.title.toLowerCase() === draft.title.toLowerCase()))
                    continue;
                ...
                if (this.memory.listMemories({ scope: "conversation", conversationId }).some((m) => m.title.toLowerCase() === draft.title.toLowerCase()))
                    continue;
