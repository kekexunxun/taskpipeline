...
function extractionPrompt(allowedScopes) {
    return [
...
export function parseExtractedMemories(text, allowedScopes) {
    ...
    try {
        ...
        for (const raw of parsed.memories) {
            ...
            const tags = Array.isArray(item.tags) ? item.tags.filter((tag) => typeof tag === "string").map((tag) => tag.trim()).filter(Boolean).slice(0, 8) : [];
            if (!allowedScopes.includes(scope) || !title || !content)
                continue;
            drafts.push({ scope: scope, title, content, tags });
        }
        return drafts;
    }
    catch {
