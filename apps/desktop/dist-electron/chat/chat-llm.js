...
export async function* streamChat(options) {
    if (options.model.provider === "qoder")
        yield* streamQoder({ model: options.model, qoderToken: options.qoderToken, messages: options.messages, signal: options.signal });
    else
        yield* streamOpenAICompatible({ model: options.model, messages: options.messages, signal: options.signal });
}
async function* streamOpenAICompatible({ model, messages, signal }) {
...
async function* streamQoder({ model, qoderToken, messages, signal }) {
    if (!qoderToken)
