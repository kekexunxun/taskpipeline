...
function resolveTemplate(issueTypeId, issueTypeName) {
    const id = issueTypeId?.trim();
    const name = issueTypeName?.trim();
    return TASK_TEMPLATES.find((template) => (id && template.issueTypeId === id) || (name && template.issueTypeName === name));
}
...
export class JiraTaskCreationAgent {
    ...
    async getCreationSchema(input) {
        ...
        if (template) {
            for (const key of template.confirmedCustomFields)
    ...
    async createJiraIssue(input) {
        ...
        if (template) {
            ...
            const components = (fields.components?.allowedValues ?? []);
            ...
            const taskLevelValues = (fields.customfield_10500?.allowedValues ?? []);
            ...
            const taskCategoryValues = (fields.customfield_12505?.allowedValues ?? []);
