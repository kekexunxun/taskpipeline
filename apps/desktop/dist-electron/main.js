...
import { resolveChatModel } from "./chat/chat-models.js";
...
const resumeImplementationInstruction = "任务此前执行失败/中断。请先检查当前工作区与代码状态（已完成的改动应保留），定位失败原因后继续完成剩余工作；不要重新执行已完成的部分，也不要重复安装依赖或重建环境。";
...
async function taskMemoryContext(task, repos) {
    try {
...
async function consolidateTaskMemory(taskId, responseTexts) {
    try {
        const task = store.getTask(taskId);
        if (!task)
            return;
...
async function consolidateChatMemory(conversation, model) {
    try {
        const text = conversation.messages
            .filter((message) => message.role !== "system")
            .map((message) => `${message.role === "user" ? "用户" : "助手"}：${message.parts.filter((part) => part.type === "text").map((part) => part.text).join("")}`)
            .join("\n\n");
        if (!text.trim())
            return;
        const extracted = await extractMemories({
...
function registerIpc() {
    ...
    ipcMain.handle("repos:delete", (_event, id) => {
    ...
    ipcMain.handle("tasks:resume", (_event, taskId) => resumeTask(taskId));
    ...
    ipcMain.handle("tasks:reveal-workspace", (_event, taskId) => {
    ...
    ipcMain.handle("memory:list", (_event, filter) => memoryService.listMemories(filter));
    ipcMain.handle("memory:upsert", (_event, input) => memoryService.upsertMemory(input));
    ipcMain.handle("memory:update", (_event, id, patch) => memoryService.updateMemory(id, patch));
    ipcMain.handle("memory:delete", (_event, id) => memoryService.deleteMemory(id));
    ipcMain.handle("memory:search", (_event, query, options) => memoryService.search({ userId: memoryService.ensureUserId(), query, ...options }));
    ipcMain.handle("repowiki:index", async (_event, repositoryId) => {
    ...
    ipcMain.handle("repowiki:list", (_event, repositoryId) => memoryService.listRepoWikiDocs(repositoryId));
    ipcMain.handle("repowiki:search", (_event, repositoryId, query) => memoryService.searchRepoWikiDocs(repositoryId, query));
    // === Chat 对话(Codex 样式) =================================================
