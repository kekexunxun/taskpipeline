...
export class TaskStore {
  ...
  constructor(filename: string) {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS tasks (
        ...
        review_status TEXT NOT NULL, commit_message TEXT, pi_session_path TEXT, qoder_model TEXT, qoder_session_id TEXT, session_usage TEXT,
      ...
      /**
       * 跨数据源归一化 trace 表（与 events 表不同：不挂 taskId，独立业务事件）。
       * 当前主用途：新增/编辑 Agent 弹窗中"AI 生成"按钮产生的模板生成记录，
       * 在 Trace 页 kind="other" 中呈现。
       */
    ...
    for (const statement of [
      ...
      'ALTER TABLE tasks ADD COLUMN qoder_session_id TEXT',
      ...
      'ALTER TABLE tasks ADD COLUMN open_code_review_enabled TEXT',
      'ALTER TABLE tasks ADD COLUMN auto_create_merge_requests TEXT',
      'ALTER TABLE tasks ADD COLUMN create_test_cases_enabled TEXT',
      'ALTER TABLE tasks ADD COLUMN tests_generated TEXT',
    ...
    try {
      ...
      if (columns.some((column) => column.name === 'jira_key')) {
        this.db.exec(
          "UPDATE tasks SET task_key = jira_key, source = 'jira' WHERE jira_key IS NOT NULL AND jira_key != '' AND task_key IS NULL"
        )
      }
    } catch {
      /* 迁移失败不影响正常打开 */
  ...
  getTask(id: string): Task | undefined {
    return this.parseTask(this.db.prepare('SELECT * FROM tasks WHERE id = ?').get(id))
  }
  ...
  listTasks(): Task[] {
    return (this.db.prepare('SELECT * FROM tasks ORDER BY updated_at DESC').all() as unknown[]).map(
      (row) => this.parseTask(row)!
    )
  }
  listCards(): TaskCard[] {
    return this.listTasks().map((task) => ({
      ...task,
      boardColumn: boardColumnFor(task.state),
      repositories: this.listTaskRepositories(task.id).map(
        ({ id, name, changeSummary, mergeRequestUrl, deliveryStatus }) => ({
          id,
          name,
          changeSummary,
          mergeRequestUrl,
  ...
  private parseTask(row: unknown): Task | undefined {
    ...
    return {
      id: String(r.id),
      taskKey: r.task_key ? String(r.task_key) : undefined,
      source: (r.source ? String(r.source) : 'local') as Task['source'],
      ...
      reviewStatus: r.review_status as Task['reviewStatus'],
      commitMessage: r.commit_message ? String(r.commit_message) : undefined,
      piSessionPath: r.pi_session_path ? String(r.pi_session_path) : undefined,
  ...
  addTaskRepository(repo: Omit<TaskRepository, 'id'>): TaskRepository {
    const item = {
  ...
  listTaskRepositories(taskId: string): TaskRepository[] {
    return this.db
      .prepare(
        'SELECT id,task_id as taskId,repository_id as repositoryId,name,local_path as localPath,base_branch as baseBranch,setup_command as setupCommand,lint_command as lintCommand,test_command as testCommand,build_command as buildCommand,feature_branch as featureBranch,worktree_path as worktreePath,change_summary as changeSummary,commit_sha as commitSha,merge_request_url as mergeRequestUrl,merge_request_iid as mergeRequestIid,merge_request_state as mergeRequestState,merge_request_checked_at as mergeRequestCheckedAt,delivery_status as deliveryStatus FROM task_repositories WHERE task_id = ?'
      )
      .all(taskId) as TaskRepository[]
  }
  attachRepository(taskId: string, repositoryId: string): TaskRepository {
    ...
    const profile = this.db
      .prepare(
        'SELECT id,name,local_path as localPath,default_branch as defaultBranch,setup_command as setupCommand,lint_command as lintCommand,test_command as testCommand,build_command as buildCommand FROM repository_profiles WHERE id = ?'
      )
      .get(repositoryId) as
      | Pick<
          RepositoryProfile,
  ...
  deleteRepositoryProfile(id: string): void {
    this.db.prepare('DELETE FROM repository_profiles WHERE id = ?').run(id)
  }
  addEvent(event: Omit<AgentEvent, 'id' | 'createdAt'>): AgentEvent {
    const item = { detail: undefined, payload: undefined, ...event, id: randomUUID(), createdAt: this.now() }
    this.db
      .prepare(
        'INSERT INTO events (id,task_id,kind,title,detail,payload,created_at) VALUES (@id,@taskId,@kind,@title,@detail,@payload,@createdAt)'
      )
  ...
  listEvents(taskId: string): AgentEvent[] {
    return (
      this.db
        .prepare(
          'SELECT id,task_id as taskId,kind,title,detail,payload,created_at as createdAt FROM events WHERE task_id = ? ORDER BY created_at ASC'
        )
        .all(taskId) as Record<string, unknown>[]
    ).map((r) => ({
      ...r,
  ...
  addApproval(input: Omit<Approval, 'id' | 'createdAt' | 'status'>): Approval {
    const item = { ...input, id: randomUUID(), status: 'pending' as const, createdAt: this.now() }
    this.db
      .prepare(
        'INSERT INTO approvals (id,task_id,kind,status,context,created_at) VALUES (@id,@taskId,@kind,@status,@context,@createdAt)'
      )
      .run(item)
    return item
  }
  listApprovals(taskId: string): Approval[] {
    return this.db
      .prepare(
        'SELECT id,task_id as taskId,kind,status,context,created_at as createdAt,resolved_at as resolvedAt FROM approvals WHERE task_id = ? ORDER BY created_at DESC'
      )
      .all(taskId) as Approval[]
  }
  resolveApproval(id: string, status: 'approved' | 'rejected'): void {
    this.db.prepare('UPDATE approvals SET status = ?, resolved_at = ? WHERE id = ?').run(status, this.now(), id)
  }
  saveRepositoryProfile(profile: RepositoryProfile): void {
    this.db
      .prepare(
        'INSERT OR REPLACE INTO repository_profiles (id,name,local_path,remote_url,default_branch,gitlab_project_id,setup_command,test_command,lint_command,build_command) VALUES (@id,@name,@localPath,@remoteUrl,@defaultBranch,@gitlabProjectId,@setupCommand,@testCommand,@lintCommand,@buildCommand)'
      )
      .run({
  ...
  listRepositoryProfiles(): RepositoryProfile[] {
    return this.db
      .prepare(
        'SELECT id,name,local_path as localPath,remote_url as remoteUrl,default_branch as defaultBranch,gitlab_project_id as gitlabProjectId,setup_command as setupCommand,test_command as testCommand,lint_command as lintCommand,build_command as buildCommand FROM repository_profiles ORDER BY name'
      )
      .all() as RepositoryProfile[]
  }
  ...
  /**
   * 写入一条 trace 业务事件（用于 Trace 页"其它"分类）。
   * 与 events 表的区别：不依赖 taskId；UI 通过 category / subType 路由到合适视图。
   */
  addTraceEvent(input: Omit<TraceEvent, 'id' | 'createdAt'>): TraceEvent {
    ...
    this.db
      .prepare(
        'INSERT INTO trace_events (id,category,sub_type,title,detail,payload,created_at) VALUES (@id,@category,@subType,@title,@detail,@payload,@createdAt)'
      )
      .run({ ...item, payload: item.payload === undefined ? undefined : JSON.stringify(item.payload) })
    return item
  }
  /** 列出所有 trace 业务事件，按时间倒序（最新在前）。 */
  listTraceEvents(): TraceEvent[] {
    return (
      this.db
        .prepare(
          'SELECT id,category,sub_type as subType,title,detail,payload,created_at as createdAt FROM trace_events ORDER BY created_at DESC'
        )
        .all() as Record<string, unknown>[]
    ).map((row) => ({
      id: String(row.id),
      category: String(row.category) as TraceEvent['category'],
  ...
  /** 按 id 查单条 trace 业务事件（Trace 详情用）。 */
  getTraceEvent(id: string): TraceEvent | undefined {
    const row = this.db
      .prepare(
        'SELECT id,category,sub_type as subType,title,detail,payload,created_at as createdAt FROM trace_events WHERE id = ?'
      )
  ...
  acquireLease(taskId: string, owner: string, ttlMs = 30_000): boolean {
    const now = Date.now()
    const tx = this.db.transaction(() => {
      this.db.prepare('DELETE FROM task_leases WHERE expires_at < ?').run(now)
      const existing = this.db.prepare('SELECT owner FROM task_leases WHERE task_id = ?').get(taskId) as
        | { owner?: string }
        | undefined
      if (existing && existing.owner !== owner) return false
      this.db
        .prepare('INSERT OR REPLACE INTO task_leases (task_id,owner,expires_at) VALUES (?,?,?)')
        .run(taskId, owner, now + ttlMs)
      return true
    })
    return tx()
  }
  releaseLease(taskId: string, owner: string): void {
    this.db.prepare('DELETE FROM task_leases WHERE task_id = ? AND owner = ?').run(taskId, owner)
  }
}
