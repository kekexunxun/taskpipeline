...
export class TaskStore {
  ...
  constructor(filename: string) {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS tasks (
        ...
        summary TEXT, review_status TEXT NOT NULL, commit_message TEXT, pi_session_path TEXT, qoder_model TEXT, session_usage TEXT,
      ...
      CREATE TABLE IF NOT EXISTS task_repositories (
        ...
        base_branch TEXT NOT NULL, feature_branch TEXT, worktree_path TEXT, change_summary TEXT,
      ...
      CREATE TABLE IF NOT EXISTS repository_profiles (
        ...
        default_branch TEXT NOT NULL, gitlab_project_id TEXT, test_command TEXT, lint_command TEXT, build_command TEXT
      );
  ...
  createTask(input: Pick<Task, "title" | "description"> & Partial<Task>): Task {
    const task: Task = {
      ...
      state: input.state ?? "draft", summary: input.summary, reviewStatus: input.reviewStatus ?? "pending",
    ...
    this.db.prepare(`INSERT INTO tasks (id,jira_key,title,description,keywords,acceptance_criteria,state,summary,review_status,commit_message,pi_session_path,qoder_model,session_usage,created_at,updated_at)
      VALUES (@id,@jiraKey,@title,@description,@keywords,@acceptanceCriteria,@state,@summary,@reviewStatus,@commitMessage,@piSessionPath,@qoderModel,@sessionUsage,@createdAt,@updatedAt)`).run({
  ...
  attachRepository(taskId: string, repositoryId: string): TaskRepository {
    ...
    const profile = this.db.prepare("SELECT id,name,local_path as localPath,default_branch as defaultBranch FROM repository_profiles WHERE id = ?").get(repositoryId) as Pick<RepositoryProfile, "id" | "name" | "localPath" | "defaultBranch"> | undefined;
    if (!profile) throw new Error(`Repository profile not found: ${repositoryId}`);
    return this.addTaskRepository({ taskId, repositoryId: profile.id, name: profile.name, localPath: profile.localPath, baseBranch: profile.defaultBranch, deliveryStatus: "pending" });
  }
