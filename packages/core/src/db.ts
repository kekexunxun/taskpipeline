...
export class TaskStore {
  ...
  createTask(input: Pick<Task, "title" | "description"> & Partial<Task>): Task {
    const task: Task = {
      ...
      commitMessage: input.commitMessage, piSessionPath: input.piSessionPath, qoderModel: input.qoderModel, qoderSessionId: input.qoderSessionId, sessionUsage: input.sessionUsage,
      openCodeReviewEnabled: input.openCodeReviewEnabled, autoCreateMergeRequests: input.autoCreateMergeRequests,
    ...
    this.db.prepare(`INSERT INTO tasks (id,task_key,source,source_url,title,description,keywords,acceptance_criteria,state,summary,start_mode,plan_content,plan_revision,failure_stage,review_status,commit_message,pi_session_path,qoder_model,qoder_session_id,session_usage,open_code_review_enabled,auto_create_merge_requests,create_test_cases_enabled,tests_generated,created_at,updated_at)
      ...
      autoCreateMergeRequests: task.autoCreateMergeRequests === undefined ? null : task.autoCreateMergeRequests ? "1" : "0",
      createTestCasesEnabled: task.createTestCasesEnabled === undefined ? null : task.createTestCasesEnabled ? "1" : "0",
  ...
  updateTask(id: string, patch: Partial<Omit<Task, "id" | "createdAt" | "updatedAt">>): Task {
    ...
    this.db.prepare(`UPDATE tasks SET task_key=@taskKey,source=@source,source_url=@sourceUrl,title=@title,description=@description,keywords=@keywords,acceptance_criteria=@acceptanceCriteria,state=@state,summary=@summary,start_mode=@startMode,plan_content=@planContent,plan_revision=@planRevision,failure_stage=@failureStage,review_status=@reviewStatus,commit_message=@commitMessage,pi_session_path=@piSessionPath,qoder_model=@qoderModel,qoder_session_id=@qoderSessionId,session_usage=@sessionUsage,open_code_review_enabled=@openCodeReviewEnabled,auto_create_merge_requests=@autoCreateMergeRequests,create_test_cases_enabled=@createTestCasesEnabled,tests_generated=@testsGenerated,updated_at=@updatedAt WHERE id=@id`).run({
      ...
      autoCreateMergeRequests: next.autoCreateMergeRequests === undefined ? null : next.autoCreateMergeRequests ? "1" : "0",
      createTestCasesEnabled: next.createTestCasesEnabled === undefined ? null : next.createTestCasesEnabled ? "1" : "0",
