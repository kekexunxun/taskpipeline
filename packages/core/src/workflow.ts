...
const transitions: Record<TaskState, TaskState[]> = {
  ...
  preparing: ['planning', 'implementing', 'failed', 'cancelled'],
  ...
  // awaiting_review -> awaiting_commit 已剪掉：它唯一的生产者是「关闭 Review」的 skip 分支，
  // 而 Review 现在是必经阶段，执行期不再有跳过它的口子。进 awaiting_commit 的合法出口只剩 reviewing。
  awaiting_review: ['reviewing', 'implementing', 'completed', 'cancelled'],
  reviewing: ['review_blocked', 'awaiting_commit', 'implementing', 'completed', 'failed'],
  review_blocked: ['reviewing', 'implementing', 'awaiting_commit', 'completed', 'cancelled'],
  awaiting_commit: ['delivering', 'implementing', 'completed', 'cancelled'],
  ...
  completed: ['preparing'],
  failed: ['preparing', 'planning', 'implementing', 'validating', 'cancelled'],
  cancelled: []
}

export function transitionTask(from: TaskState, to: TaskState): void {
  if (!TASK_STATES.includes(to) || !transitions[from].includes(to))
    throw new Error(`Invalid task transition: ${from} -> ${to}`)
}

export function transitionTaskSafely(from: TaskState, to: TaskState): TaskState {
...
export function isReviewable(state: TaskState): boolean {
  return REVIEWABLE_STATES.includes(state)
}
export function isDeliverable(state: TaskState): boolean {
  return DELIVERABLE_STATES.includes(state)
}
export function isMergeTrackable(state: TaskState): boolean {
  return MERGE_TRACKABLE_STATES.includes(state)
}
