...
const dirs: string[] = [];
afterEach(() => { delete process.env.CODING_AGENT_TEST_KEY; for (const dir of dirs.splice(0)) rmSync(dir, { recursive: true, force: true }); });

describe("LocalFileKeyStore", () => {
  it("encrypts with AAD and creates a user-only installation key", () => {
    const dir = mkdtempSync(join(tmpdir(), "coding-agent-key-")); dirs.push(dir);
  ...
  it("resolves environment references without persistence", () => {
    const dir = mkdtempSync(join(tmpdir(), "coding-agent-key-")); dirs.push(dir);
    process.env.CODING_AGENT_TEST_KEY = "from-env";
    expect(new LocalFileKeyStore(dir).resolve("env:CODING_AGENT_TEST_KEY")).toBe("from-env");
  });
});
