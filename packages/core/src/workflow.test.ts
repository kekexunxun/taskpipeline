...
describe('task workflow', () => {
  ...
  it('rejects the retired skip-review edge', () => {
    // awaiting_review -> awaiting_commit 曾专为「关闭 Review」分支存在；Review 必经后该边必须抛错，
    // 否则 task-runner 里残留的 skip 分支会静默把任务推到可提交状态。
    expect(() => transitionTask('awaiting_review', 'awaiting_commit')).toThrow(/Invalid task transition/)
  })
