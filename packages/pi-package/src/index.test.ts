...
vi.mock('@task-pipeline/core', () => {
  ...
  return {
    ...
    LocalFileKeyStore: class {
      resolve = id
    },
    // tool_call 权限判定已上移到 core；本测试不跑到 handler，给个恒放行替身即可。
    evaluateExecutionPermission: () => ({ action: 'allow' }),
    taskRoots: (repos: Array<{ worktreePath?: string; localPath: string }>) =>
      repos.map((r) => r.worktreePath ?? r.localPath)
  }
})

vi.mock('@task-pipeline/integrations', () => ({
  AtlassianClientFactory: class {
    create() {
      return {}
    }
  },
  ...
  MergeStatusRefresher: class {
    constructor() {}
  },
  OpenAICompatReviewer: class {
    constructor() {}
  },
  OpenCodeReviewService: class {
    constructor() {}
  },
