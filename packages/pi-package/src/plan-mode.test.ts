...
describe("PiAgentPlanModeProvider.parseOutput", () => {
  ...
  it("解析 changes_required", () => {
    expect(provider.parseOutput('前缀文本 {"outcome":"changes_required","plan":"do X"} 后缀'))
...
describe("buildPlannerEnv", () => {
  ...
  it("drop 宿主凭据:JIRA_TOKEN / DOCKER_* / *_SECRET / ATLAS", () => {
    ...
    setEnv("ATLAS_URI", "mongodb://host:27017");
    setEnv("CODING_AGENT_DATA_DIR", "/private/data");
    const env = buildPlannerEnv();
    ...
    expect(env.CODING_AGENT_DATA_DIR).toBeUndefined();
  });
...
describe("PiAgentPlanModeProvider.runPlan", () => {
  ...
  it("spawn 收到 --tools read,grep,find,ls + CODING_AGENT_SUBAGENT=1", async () => {
    ...
    expect((opts as { env: Record<string, string> }).env.CODING_AGENT_SUBAGENT).toBe("1");
  ...
  it("L7: spawn 包含 --subagent-nonce 且与 env 中 CODING_AGENT_SUBAGENT_NONCE 一致", async () => {
    const child = makeFakeChild({ spawnargs: [], spawnfile: "" });
    const promise = runProvider(child);
    await waitForSpawn();
    queueMicrotask(() => {
    ...
    expect(opts.env.CODING_AGENT_SUBAGENT_NONCE).toBe(flagNonce);
  });
  ...
  it("L5: spawn env 净化:宿主凭据 JIRA_TOKEN / DOCKER_* 不会透传到子进程", async () => {
    ...
    try {
      ...
      // 标记位仍然存在
      expect(opts.env.CODING_AGENT_SUBAGENT).toBe("1");
    } finally {
  ...
  it("signal abort 触发 kill('SIGTERM')", async () => {
    ...
    const promise = provider.runPlan(TEST_CTX, { cwd: "/tmp/workspace", hardTimeoutMs: 60_000, signal: controller.signal });
    await waitForSpawn();
    controller.abort();
  ...
  it("spawn 失败时清理 tmp 目录", async () => {
    ...
    const cleanupCall = rmSpy.mock.calls.find((call) => {
      ...
      return /coding-agent-planner-/.test(arg);
    });
