 ...
 * - 显式标记: CODING_AGENT_SUBAGENT + CODING_AGENT_SUBAGENT_NONCE(在 spawn env 里硬设)
 *
 * **不放** 的:JIRA_TOKEN / DOCKER_* / CODING_AGENT_DATA_DIR / 任何 *_SECRET / 任何
...
export class PiAgentPlanModeProvider implements PlanModeProvider {
  ...
  private async spawnPlanner(
    ...
    const tmpDir = await mkdtemp(join(tmpdir(), "coding-agent-planner-"));
    ...
    const args: string[] = [
      ...
      "--tools", [...PLANNER_TOOLS].join(","),
      "--subagent-nonce", nonce      // 传给子进程,子进程 guard 验
    ];
    ...
    const proc: ChildProcess = spawn(process.execPath, [scriptPath, ...args, userMessage], {
      ...
      env: {
        ...sanitizedEnv,
        CODING_AGENT_SUBAGENT: "1",
        CODING_AGENT_SUBAGENT_NONCE: nonce
      },
