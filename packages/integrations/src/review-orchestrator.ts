 ...
 */
function readDefaultOpenAIProfile(
  resolver: SettingResolver
): { id?: string; baseUrl: string; model: string; apiKeyEnv?: string; isDefault?: boolean } | undefined {
  ...
  try {
    ...
    if (profile.baseUrl && profile.model)
      return { ...profile, isDefault: true } as {
        id?: string
        baseUrl: string
        model: string
        apiKeyEnv?: string
 ...
 */
export function asReviewer(fn: ReviewerFunction): Reviewer {
...
export function parseReviewResult(text: string): ReviewResult {
  ...
  try {
    ...
    throw new Error(
      `LLM 审查响应中找不到 JSON 对象: ${(extractError as Error).message}; 原始输出前 1000 字符: ${trimmed.slice(0, 1000)}`
    )
  }
  try {
    ...
    throw new Error(
      `LLM 审查响应不是合法 JSON: ${(error as Error).message}; 提取到的 JSON 前 1000 字符: ${json.slice(0, 1000)}; 原始输出前 1000 字符: ${trimmed.slice(0, 1000)}`
    )
  }
}
...
export class OpenAICompatReviewer implements Reviewer {
  constructor(
    ...
    private readonly timeoutMs: number = 3 * 60_000,
  ...
  call = async (
    ...
    prompt?: string,
    /** 可选：响应 usage 透传（trace 埋点记录 token 用量用；不影响返回值）。 */
    ...
    const timer = setTimeout(
      () => abort.abort(new Error(`OpenAI 兼容 Review 在 ${this.timeoutMs / 1000}s 内未返回,主动 abort`)),
      this.timeoutMs
    )
    try {
      ...
      const u = json.usage
      if (onUsage && u && typeof u === 'object') {
        const inputTokens = u.prompt_tokens ?? 0
        const outputTokens = u.completion_tokens ?? 0
        if (inputTokens + outputTokens > 0) {
          onUsage({ inputTokens, outputTokens, totalTokens: u.total_tokens ?? inputTokens + outputTokens })
        }
      }
...
export class ReviewOrchestrator {
  ...
  async run(task: Task, repo: TaskRepository, signal?: AbortSignal): Promise<ReviewResult> {
    ...
    this.sink.addEvent({
      taskId: task.id,
      kind: 'status',
      title: `${repo.name}: 取变更文件列表 (base=${repo.baseBranch})`
    })
    ...
    this.sink.addEvent({
      taskId: task.id,
      kind: 'status',
      title: `${repo.name}: 找到 ${files.length} 个变更文件,取 diff`
    })
    let diff = ''
    try {
    ...
    if (diff.length === 0) {
      this.sink.addEvent({
        taskId: task.id,
        kind: 'status',
        title: `${repo.name}: diff 为空 (worktree 没有相对 ${repo.baseBranch} 的提交变更),跳过 LLM`,
        detail: '如果预期应该有 diff,先在 worktree 里 commit 或 push 再重试 review'
      })
      return { status: 'completed', comments: [], summary: { files: files.length, comments: 0 } }
    }
    this.sink.addEvent({
