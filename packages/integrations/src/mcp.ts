...
export class McpClient {
  ...
  async connect(): Promise<void> {
    if (this.profile.transport === 'stdio') {
      ...
      try {
        ...
        throw new Error(
          `无法启动 MCP 子进程（${this.profile.command}）：${error instanceof Error ? error.message : String(error)}`
        )
      }
      this.child = child
      ...
      lines.on('line', (line) => {
        try {
          ...
          if (response.id !== undefined) this.settle(response.id, response)
        } catch {
          /* Server logs are ignored; protocol lines remain parseable. */
        }
      })
  ...
  private headers(): Record<string, string> {
    ...
    if (token)
      headers[this.profile.tokenHeader ?? 'Authorization'] =
        (this.profile.tokenHeader ?? 'Authorization').toLowerCase() === 'authorization' ? `Bearer ${token}` : token
  ...
  private async connectLegacySse(): Promise<void> {
    ...
    const response = await this.fetcher(this.profile.url!, {
      ...
      while (true) {
        ...
        while ((boundary = buffer.indexOf('\n\n')) >= 0) {
          ...
          try {
            ...
            /* Ignore non-protocol SSE diagnostics. */
          }
        }
      }
    })().catch((error) => {
    ...
    try {
      await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          readyTimer = setTimeout(() => reject(new Error('MCP SSE endpoint timeout')), 30_000)
        })
      ])
    } finally {
  ...
  private async legacySseSend(payload: Record<string, unknown>): Promise<void> {
    ...
    const response = await this.fetcher(this.sseEndpoint, {
      method: 'POST',
  ...
  async callTool(name: string, arguments_: Record<string, unknown>): Promise<unknown> {
    if (!this.initialized) await this.connect()
    return this.runRequest('tools/call', { name, arguments: arguments_ })
  }
