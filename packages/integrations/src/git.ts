...
export class GitService {
  ...
  constructor(
    private readonly run: GitRunner = async (args, cwd, timeoutMs, signal) =>
      (
  ...
  private async fetchRemote(cwd: string, signal?: AbortSignal): Promise<string> {
    try {
      ...
      if (
        failure.timedOut ||
        /timed out|Could not resolve host|Failed to connect|Connection refused|Network is unreachable|No route to host|ssh: connect to host/i.test(
          detail
        )
      ) {
      ...
      if (
        /Authentication failed|Access denied|Permission denied|could not read Username|terminal prompts disabled/i.test(
          detail
        )
      ) {
  ...
  /**
   * 仅查看工作区变更（不依赖 baseBranch），用于对话级文件变更展示。
   * 合并 staged + unstaged 状态，按路径排序返回。
   */
  ...
  async changedFiles(cwd: string, baseBranch: string, signal?: AbortSignal): Promise<GitChangedFile[]> {
    ...
    const files = new Map<string, GitChangedFile>()
    try {
      for (const file of parseNameStatus(
        await this.run(['diff', '--name-status', '-z', `${baseRef}...HEAD`], cwd, undefined, signal)
      ))
        files.set(file.path, file)
    } catch {
  ...
  async inspectRepository(cwd: string): Promise<GitRepositoryInfo> {
    ...
    try {
      remoteUrl = (await this.run(['config', '--get', 'remote.origin.url'], cwd)).trim() || undefined
    } catch {
      /* A local-only repository may not have an origin. */
    }
  ...
  async push(cwd: string, branch: string, token?: string, signal?: AbortSignal): Promise<string> {
    try {
      return await this.run(
        [
          ...(token ? ['-c', `http.extraHeader=PRIVATE-TOKEN: ${token}`] : []),
          'push',
          '--set-upstream',
          'origin',
          branch
      ...
      if (
        /Connection timed out|Could not resolve host|Failed to connect|Connection refused|Network is unreachable|fatal: unable to access/.test(
          combined
        )
      ) {
        const remoteUrl = await this.run(['config', '--get', 'remote.origin.url'], cwd).catch(() => '<unknown>')
        throw new Error(
          `无法连接到 GitLab (${remoteUrl.trim()}): 网络不通或地址不可达,请检查网络 / VPN。\n\n原始错误: ${execaError.message ?? combined}`
        )
      }
      throw error
    }
  }
  removeWorktree(repoPath: string, worktreePath: string): Promise<string> {
    return this.run(['worktree', 'remove', '--force', worktreePath], repoPath)
  }
  pruneWorktrees(repoPath: string): Promise<string> {
    return this.run(['worktree', 'prune'], repoPath)
  }
