...
describe('Jira mapping', () => {
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify({
            data: { tickets: [{ id: 'PAY-7', subject: 'Fix checkout', body: 'Race', tags: ['pay'], state: 'Doing' }] }
          })
        }
      ]
    }
    const tasks = mapJiraTasks(response, {
      itemsPath: 'data.tickets',
      fields: { key: 'id', title: 'subject', description: 'body', keywords: 'tags', status: 'state' },
      statusMap: { Doing: 'implementing' }
    })
  ...
  it('maps the simplified issue shape returned by mcp-atlassian', () => {
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ...
            issues: [
              {
                key: 'OPS-12',
                self: 'https://jira.example.com/rest/api/2/issue/OPS-12',
                summary: 'Fix export',
                description: 'Include audit fields',
                labels: ['audit'],
                status: { name: 'Open' }
              }
            ]
    ...
    expect(mapJiraTasks(response)).toEqual([
      {
        taskKey: 'OPS-12',
        source: 'jira',
...
describe('MCP', () => {
  it('initializes Streamable HTTP and forwards an environment token', async () => {
    ...
    const fetcher = async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify(
          body.id ? { jsonrpc: '2.0', id: body.id, result: body.method === 'tools/call' ? { content: [] } : {} } : {}
        ),
    ...
    const client = new McpClient(
      ...
      { TEST_MCP_TOKEN: 'token-value' },
      fetcher as typeof fetch
    )
    await client.callTool('search', { query: 'mine' })
    client.close()
  ...
  it('connects to a legacy SSE endpoint and receives tool responses', async () => {
      ...
      if (body.id) {
        ...
        queueMicrotask(() =>
          streamController?.enqueue(
            encoder.encode(`data: ${JSON.stringify({ jsonrpc: '2.0', id: body.id, result })}\n\n`)
          )
        )
      }
    ...
    await expect(client.callTool('search', { query: 'mine' })).resolves.toEqual({
      content: [{ type: 'text', text: 'ok' }]
    })
    client.close()
    expect(
      posts.some((call) => call.url === 'https://jira-mcp.internal/messages' && call.body.method === 'tools/call')
    ).toBe(true)
...
describe('process integrations', () => {
  it('parses Open Code Review JSON through an injected runner', async () => {
    const review = new OpenCodeReviewService('ocr', async () => ({
      stdout: JSON.stringify({ status: 'completed', comments: [{ severity: 'high' }] }),
      stderr: '',
      exitCode: 0
    }))
    await expect(review.review(process.cwd())).resolves.toMatchObject({
      status: 'completed',
  ...
  it('includes execa failure reason when ocr cannot be launched', async () => {
    const service = new OpenCodeReviewService('ocr-missing', async () => ({
      stdout: '',
      stderr: '',
      exitCode: 1,
      failed: true,
  ...
  it('skips the rule subprocess when there are no paths', async () => {
    let called = false
    const service = new OpenCodeReviewService('ocr', async () => {
      called = true
  ...
  it('creates a branch worktree with the expected Git sequence', async () => {
    ...
    const git = new GitService(async (args, _cwd, timeoutMs) => {
      calls.push(args)
    ...
    expect(calls).toEqual([
      ['fetch', '--all', '--prune'],
      ['worktree', 'add', '-B', 'agent/ABC-1-fix', '/worktrees/agent-ABC-1-fix', 'main']
    ])
  ...
  it('reports fetch timeouts as a retryable remote connection error', async () => {
    const git = new GitService(async (args) => {
      if (args[0] === 'fetch')
        throw Object.assign(new Error('Command timed out after 60000 milliseconds'), { timedOut: true })
  ...
  it('creates a task branch named after its Jira key from the configured base branch', async () => {
    ...
    const worktree = await git.createTaskWorktree(
      '/repo',
      '/workspaces/task-1',
      'ABC-123',
      'release/next',
  ...
  it('fails instead of falling back to a local branch when the configured remote base is missing', async () => {
    ...
    await expect(git.createTaskWorktree('/repo', '/workspaces/task-1', 'ABC-123', 'main')).rejects.toThrow(
      'refs/remotes/origin/main'
    )
  ...
  it('reuses the existing HEAD when worktree is already clean (retry after commit-succeeded-but-push-failed)', async () => {
    const calls: string[][] = []
    const execaError = Object.assign(new Error("Command failed with exit code 1: git commit --no-verify -m '...'"), {
      exitCode: 1,
      stdout: 'On branch agent/bsadapt344-36488-adaptor\nnothing to commit, working tree clean\n',
      stderr: ''
    })
    const git = new GitService(async (args) => {
      calls.push(args)
  ...
  it('still throws on real commit failures (e.g. husky rejects the commit message)', async () => {
    ...
    const execaError = Object.assign(new Error("Command failed with exit code 1: git commit --no-verify -m '...'"), {
      exitCode: 1,
      stdout: 'husky > commit-msg hook failed',
      stderr: ''
    })
    const git = new GitService(async (args) => {
      if (args[0] === 'commit') throw execaError
    ...
    const git = new GitService(async (args) => {
      ...
      if (args.includes('push')) {
        const err = Object.assign(new Error('Command failed: git push'), {
          ...
          stdout: '',
          stderr:
            "fatal: unable to access 'http://192.168.175.129:90/adaptor/adaptor-suit-front.git/': Failed to connect to 192.168.175.129 port 90: Connection timed out"
        })
        throw err
      }
      if (args[0] === 'config') {
        configGetCalled = true
        return 'http://192.168.175.129:90/adaptor/adaptor-suit-front.git\n'
      }
      return ''
    })
  ...
  it('leaves non-network push errors untouched (e.g. auth rejected)', async () => {
    ...
    const git = new GitService(async (args) => {
      ...
      const err = Object.assign(new Error('Command failed: git push'), {
        exitCode: 128,
        stdout: '',
        stderr: 'remote: HTTP Basic: Access denied. The provided password or token is incorrect.'
      })
      throw err
    })
...
describe('task command workflow', () => {
  it('lets a task override clear the repository default, and defers setup to plan approval', async () => {
    ...
    try {
      ...
      const taskRepo = store.addTaskRepository({
        taskId: task.id,
        repositoryId: 'repo',
        name: 'repo',
        localPath: dir,
      ...
      await expect(
        workflow.begin(task.id, { repositoryCommands: { repo: { setupCommand: '' } } })
      ).resolves.toMatchObject({ state: 'planning' })

      // 固定链路：begin 只建环境，准备命令要等计划批准后才跑。
      expect(calls).toEqual([])
      expect(store.listTaskRepositories(task.id).find((repo) => repo.id === taskRepo.id)?.setupCommand).toBe('')

      workflow.setPlan(task.id, '1. 无需改动')
      await expect(workflow.approvePlan(task.id)).resolves.toMatchObject({ state: 'implementing' })
      // 覆盖成空命令后，批准也不应该跑任何准备命令。
      expect(calls).toEqual([])
      store.close()
    } finally {
  ...
  it('enters planning from begin() regardless of caller intent', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'task-pipeline-workflow-'))
    try {
      const store = new TaskStore(join(dir, 'store.db'))
      store.saveRepositoryProfile({ id: 'repo', name: 'repo', localPath: dir, defaultBranch: 'main' })
      const task = store.createTask({ title: 'Plan entry', description: 'test' })
      store.addTaskRepository({
        taskId: task.id,
        repositoryId: 'repo',
        name: 'repo',
        localPath: dir,
      ...
      await expect(workflow.begin(task.id)).resolves.toMatchObject({ state: 'planning' })
      // 缺省即固定链路的必经入口：begin 不跑 setup（它留给计划批准后的 runSetup），也不跳入 implementing。
      expect(calls).toEqual([])
      store.close()
    } finally {
      rmSync(dir, { recursive: true, force: true })
  ...
  it('marks environment preparation as failed and retries a missing worktree', async () => {
    ...
    try {
      ...
      await expect(workflow.begin(task.id)).rejects.toThrow(/准备任务环境失败.*Git 拉取超时/)
      expect(store.getTask(task.id)).toMatchObject({ state: 'failed', failureStage: 'preparing' })
      expect(
        store
          .listEvents(task.id)
          .some((event) => event.title === '任务环境准备失败' && event.detail?.includes('Git 拉取超时'))
      ).toBe(true)

      await expect(workflow.begin(task.id)).resolves.toMatchObject({ state: 'planning', failureStage: undefined })
      expect(attempts).toBe(2)
      expect(store.listTaskRepositories(task.id)[0]).toMatchObject({
        worktreePath: join(dir, 'repo-worktree'),
        featureBranch: 'task-branch'
      })
      store.close()
    } finally {
  ...
  it('resets a completed task for reimplementation and preserves the old MR in history', () => {
    ...
    try {
      ...
      store.addTaskRepository({
        ...
        localPath: dir,
        baseBranch: 'main',
        mergeRequestUrl: 'https://gitlab.example.com/group/repo/-/merge_requests/7',
        mergeRequestIid: 7,
      ...
      expect(workflow.reimplement(task.id)).toMatchObject({
        state: 'preparing',
  ...
  it('still enters validating but logs a no-op when no command is configured', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'task-pipeline-workflow-'))
    try {
      const store = new TaskStore(join(dir, 'store.db'))
      const task = store.createTask({ title: 'No commands', description: 'test', state: 'implementing' })
      store.addTaskRepository({
        taskId: task.id,
        repositoryId: 'repo',
        name: 'repo',
      ...
      // 「必经」= 必然进入该 state，不等于必然执行动作；但没做必须留痕（§2.2）。
      await expect(workflow.runValidation(task.id)).resolves.toMatchObject({ state: 'awaiting_review' })
      expect(calls).toEqual([])
      const titles = store.listEvents(task.id).map((event) => event.title)
      expect(titles).toContain('未配置测试命令,本阶段只跑 Lint / Build')
      expect(titles).toContain('未配置校验命令,跳过校验')
      store.close()
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  ...
  it("treats an all-unchanged repository set as Review's only allowed no-op", async () => {
    const dir = mkdtempSync(join(tmpdir(), 'task-pipeline-workflow-'))
    try {
      const store = new TaskStore(join(dir, 'store.db'))
      const task = store.createTask({ title: 'Unchanged review', description: 'test', state: 'implementing' })
      store.addTaskRepository({
        taskId: task.id,
        repositoryId: 'repo',
        name: 'repo',
      ...
      await workflow.runReview(task.id, { run, isBlockingComment: () => false } as any)
      expect(reviewRuns).toBe(0)
      expect(store.getTask(task.id)).toMatchObject({ state: 'awaiting_commit', reviewStatus: 'passed' })
      expect(store.listEvents(task.id).some((event) => event.title === '无代码改动,Review 跳过')).toBe(true)
      store.close()
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
...
describe('GitLab remote parsing', () => {
  it('derives API host and project path from HTTPS and SSH remotes', () => {
    expect(parseGitLabRemote('https://gitlab.example.com/group/service.git')).toEqual({
      baseUrl: 'https://gitlab.example.com',
      projectId: 'group/service'
    })
    expect(parseGitLabRemote('git@gitlab.example.com:group/service.git')).toEqual({
      baseUrl: 'https://gitlab.example.com',
      projectId: 'group/service'
    })
  })
